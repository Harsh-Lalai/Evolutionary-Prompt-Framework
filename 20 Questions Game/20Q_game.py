...
def rogue_score():
    ...
    The index of the prompt is used as the key in the JSON file.
    """
    # Load the current league prompts
    df = pd.read_csv(f"prompts_{current_league}.csv")
    prompts = df["Prompt"].to_list()
    ...
    # Process Prompts
    for i in range(len(prompts) - 1):
        for j in range(i + 1, len(prompts)):  # Compare prompt i with each subsequent prompt (i+1, i+2, ...)
            # Check if the key for this specific prompt pair already exists in the JSON file
            key = f"{i}_{j}"
            ...
            # Compute the BERT score between prompt[i] and prompt[j]
            results = rogue.compute(predictions=[prompts[i]], references=[prompts[j]])
    ...
    for i in range(1, current_league):
        if os.path.exists(f"prompts_{current_league-i}.csv"):
            ...
            # Process cross-league prompts
            for j in range(len(prompts)):
                for k in range(len(prompts_prev)):  # Compare prompt[j] with each previous league prompt[k]
                    key = f"{j}_{k}"
                    ...
                    # Compute the BERT score between prompt[j] and prompt_prev[k]
                    results = rogue.compute(predictions=[prompts[j]], references=[prompts_prev[k]])
...
def tournament(no_of_objects, no_of_prompts_start, no_of_prompts_between, k, n, temp, max_tokens, current_directory):
        ...
        while(consecutive_fails<3):
            ...
            if (current_league ==1):
                ...
                top_k_prompts_league1, bottom_k_prompts_league1 = league(prompts, objects, k, current_league, temp, max_tokens, True, file_path)
            ...
            for k in range (5,6,1):
                for temperature in temp:
                    for max_token in max_tokens:
                        ...
                        folder_name = "no_of_prompts_start_"+str(no_of_prompts_start)+"_no_of_prompts_between_"+str(no_of_prompts_between)+"_k_"+str(k)+"_temperature_"+str(temperature)+"_max_token_"+str(max_token)+"_no_of_objects_"+str(no_of_objects)+"_Human_start"
