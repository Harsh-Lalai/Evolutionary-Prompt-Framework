...
def get_prompts(no_of_prompts, league_no, temp, max_tokens, top_k_prompts=None, bottom_k_prompts=None, human=False, human_file=None):
    ...
    while(features==None):
        ...
        # Check if features.json file already exists
        if os.path.exists("features.json"):
            with open("features.json", "r") as f:
                data = json.load(f)
                if str(league_no) in data:
                    features = data[str(league_no)]
                    break
        ...
        response = api_call_openai("gpt-4o-mini-2024-07-18", instructions_for_campaign_speech2(top_k_prompts) ,temp, max_tokens)
        # Extract the features
        features = extract_strategy(response)
        if features is None:
            print(response)
            times_runned += 1
            print("Features are None, retrying...")
            continue

        print("Features extracted successfully!", features)
        # Store the features in a json file with the league number as key and value as the feature. First check if the features.json file already exists
        if os.path.exists("features.json"):
            # Check if the correspinding league number is present already, if yes then use that as features instead of generating
            with open("features.json", "r") as f:
                data = json.load(f)
                if str(league_no) in data:
                    features = data[str(league_no)]
                else:
                    data[str(league_no)] = features
                    with open("features.json", "w") as f:
                        json.dump(data, f)
        else:
            with open("features.json", "w") as f:
                json.dump({str(league_no): features}, f)
    ...
    while not check_prompts(no_of_prompts, f"prompts_{league_no}.csv"):
        ...
        while(len(candidate_prompts_set)<4):
            ...
            json_response = api_call_openai_json("gpt-4o-mini-2024-07-18", instruction_for_campaign_speech3(features), temp, max_tokens)
...
def instruction_for_campaign_speech1(existing_prompts=None, candidates=None, top_k_prompts=None, bottom_k_prompts=None):
    ...
    if bottom_k_prompts:
        bottom_prompts_text = "\n".join([f'{i+1}. "{prompt}"' for i, prompt in enumerate(bottom_k_prompts)])
    ...
    if top_k_prompts is None and bottom_k_prompts is None:
        ...
        if not existing_prompts:
            instructions = (
                "Help me create a new and effective prompt to guide an AI model in generating a campaign speech for a candidate for presidency aiming to secure majority votes.\n\n"
                "We have a framework that evaluates the effectiveness of prompts.\n"
                "Here are some of the most and least effective prompts already generated:\n\n"
