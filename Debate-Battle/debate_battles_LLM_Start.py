...
def api_call_openai_json(model_name, prompt, temp, max_tokens):
    try:
        ...
        # Return an empty dictionary or some default JSON structure
        return None
    ...
    while not check_prompts(no_of_prompts, f"prompts_{league_no}.csv", role):
        ...
        while(len(candidate_prompts_set)<4):
            ...
            if json_response is None:
                times_runned += 1
                print("Response not in correct format, retrying...")
                continue
...
def bert_score():
    ...
    for i in range(len(pro_prompts) - 1):
        ...
        if key_exists_in_json(f"bert_score_{current_league}_pro.json", i):
        ...
        if os.path.exists(f"bert_score_{current_league}_pro.json"):
            with open(f"bert_score_{current_league}_pro.json", "r") as f:
                data = json.load(f)
        ...
        with open(f"bert_score_{current_league}_pro.json", "w") as f:
            json.dump(data, f)
    ...
    for i in range(1, current_league):
        if os.path.exists(f"prompts_{current_league-i}.csv"):
            ...
            for j in range(len(pro_prompts)):
                if key_exists_in_json(f"bert_score_{current_league}_pro_{current_league-i}.json", j):
                ...
                # Check if the JSON file exists and update it
                if os.path.exists(f"bert_score_{current_league}_pro_{current_league-i}.json"):
                    with open(f"bert_score_{current_league}_pro_{current_league-i}.json", "r") as f:
                        data = json.load(f)
                ...
                # Write back to the JSON file
                with open(f"bert_score_{current_league}_pro_{current_league-i}.json", "w") as f:
                    json.dump(data, f)
...
Overall Impact: Reflect on the overall persuasiveness and memorability of each debater's case. Evaluate which debater presented a more convincing and compelling argument overall."""
        ...
        for i in range(1, 8):
            topic = random.choice(topic_humans)
            best_of_wins_pro = 0
            best_of_wins_con = 0
            for j in range(1,4):
                times_runned = 0
                ...
                if self_consis_pro > self_consis_con:
                    best_of_wins_pro += 1
                elif self_consis_pro < self_consis_con:
                    best_of_wins_con += 1
            
            if best_of_wins_pro > best_of_wins_con:
                count_wins_pro += 1
                winner_list[i-1] = "Pro"
            elif best_of_wins_pro < best_of_wins_con:
                count_wins_con += 1
                winner_list[i-1] = "Con"
        
        # Determine the overall winner of the Debate Battle
        if count_wins_pro > count_wins_con:
            winner = "Pro"
        elif count_wins_pro < count_wins_con:
            winner = "Con"

        end_time = time.time()
        print(f"Time taken to complete the game: {end_time - start_time} seconds")
        results = [pro_prompt, con_prompt, topic, total_rounds, pro_arguments, con_arguments, winner, reason, count_wins_pro, count_wins_con]
        return results
...
def elo_rating(pro_prompt, con_prompt, topic, total_rounds, temp, max_tokens, all_topics, game_over = False, results = None, custom_dict = None):
    ...
    # Calculate the weighting factor for ELO update based on win proportions
    total_games = pro_wins + con_wins
    pro_weight = pro_wins / total_games if total_games > 0 else 0.5
    con_weight = con_wins / total_games if total_games > 0 else 0.5
    ...
    if (custom_dict == None):
        ...
        if winner == "draw":
            ...
            winner_rating += 40 * (0.5 - expected_winner) * pro_weight
            loser_rating +=  40 * (0.5 - expected_loser) * con_weight
        ...
        #If the winner is the Pro side debater, then calculate the ELO ratings of the prompts and print the updated ELO ratings
            ...
            expected_loser = 1 / (1 + 10 ** ((winner_rating - loser_rating) / 400))
            # Update the ELO ratings of the winner and loser
            winner_rating += 40 * (1 - expected_winner) * pro_weight
            loser_rating += 40 * (0 - expected_loser) * con_weight
            # Update the ELO ratings in the dictionary
            elo_ratings_dict[pro_prompt] = winner_rating
            elo_ratings_dict[con_prompt] = loser_rating
            # # Print the updated ELO ratings
        ...
        #If the winner is the Con side debater, then calculate the ELO ratings of the prompts and print the updated ELO ratings
            ...
            expected_loser = 1 / (1 + 10 ** ((winner_rating - loser_rating) / 400))
            # Update the ELO ratings of the winner and loser
            winner_rating +=  40 * (1 - expected_winner) * con_weight
            loser_rating += 40 * (0 - expected_loser) * pro_weight
            # Update the ELO ratings in the dictionary
...
def tournament(no_of_topics, no_of_prompts_start, no_of_prompts_between, total_rounds, k, n, temp, max_tokens, current_directory):
        ...
        # run_final_leagues(no_of_topics, n, k, total_rounds, temp, max_tokens, file_path_2)
...
# def run_final_leagues(no_of_topics, n, k, total_rounds, temp, max_tokens, file_path):

#     # Read the league_results.json file
#     with open('league_results.json', 'r') as json_file:
#         data = json.load(json_file)
        
#     topics = get_debate_topic(no_of_topics, temp, max_tokens, file_path)
...
#         battle_number = 0
#         # Collect all the pro and con prompts from the previous leagues of i using league_resutls.json file along with their elo scores
#         all_pro_prompts = {}
#         all_con_prompts = {}
#         for league_no in range(1, z):
#             all_pro_prompts[league_no] = {}
#             all_con_prompts[league_no] = {}  
#             # Store all the prompts with their final ELO scores
#             for battle_no, battle_dict in data[str(league_no)].items():
#                 pro_prompt = battle_dict['pro-prompt']['prompt']
#                 con_prompt = battle_dict['con-prompt']['prompt']
#                 pro_final_elo = battle_dict['pro-prompt']['final_elo']
#                 con_final_elo = battle_dict['con-prompt']['final_elo']
#                 all_pro_prompts[league_no][pro_prompt] = pro_final_elo
#                 all_con_prompts[league_no][con_prompt] = con_final_elo
#             # Store the top k prompts from each league in the specific_elo_ratings_dict
#             # First sort the prompts based on their ELO scores to find top k prompts for each role
#             sorted_prompts_pro = sorted(all_pro_prompts[league_no].items(), key=lambda x: x[1], reverse=True)
#             sorted_prompts_con = sorted(all_con_prompts[league_no].items(), key=lambda x: x[1], reverse=True)
#             # Get the top k prompts for the Pro side debater
#             top_k_prompts_pro = sorted_prompts_pro[:k]
#             # Get the top k prompts for the Con side debater
#             top_k_prompts_con = sorted_prompts_con[:k]
#             for prompt, elo in top_k_prompts_pro:
#                 specific_elo_ratings_dict[prompt] = elo
#             for prompt, elo in top_k_prompts_con:
#                 specific_elo_ratings_dict[prompt] = elo
#             # Append the top k prompts for each role in two lists respectively without the elo score
#             for prompt, elo in top_k_prompts_pro:
#                 top_prompts_pro.append(prompt)
#             for prompt, elo in top_k_prompts_con:
#                 top_prompts_con.append(prompt)
            
    
#         # Generate the round robin list with pair (i, j)
#         round_robin_list = [(i, j) for i in range(len(top_prompts_pro)) for j in range(len(top_prompts_con))]
#         random.shuffle(round_robin_list)

#         # To store the results after parallel processing
...
#         with concurrent.futures.ThreadPoolExecutor(max_workers=7) as executor:
#             future_to_battle = {
#                 executor.submit(game, top_prompts_pro[i], top_prompts_con[j], random.choice(topics), total_rounds, temp, max_tokens): (index, i, j)
#                 for index, (i, j) in enumerate(round_robin_list)
#             }
...
#         for (i, j, result) in results_list:
#             if result is None:
#                 print(f"Game between {top_prompts_pro[i]} and {top_prompts_con[j]} failed.")
#                 result = [top_prompts_pro[i], top_prompts_con[j], topic, total_rounds, [], [], "draw"]
#                 final_elo_rating1 = initial_elo_rating_1
#                 final_elo_rating2 = initial_elo_rating_2
#                 winner = "draw"
#             else:
#                 topic = result[2]
#                 initial_elo_rating_1 = specific_elo_ratings_dict[top_prompts_pro[i]]
#                 initial_elo_rating_2 = specific_elo_ratings_dict[top_prompts_con[j]]
#                 winner = result[6]
#                 specific_elo_ratings_dict = update_elo(specific_elo_ratings_dict, top_prompts_pro[i], top_prompts_con[j], winner)
#                 final_elo_rating1 = specific_elo_ratings_dict[top_prompts_pro[i]]
#                 final_elo_rating2 = specific_elo_ratings_dict[top_prompts_con[j]]

#             battle_number += 1
#             # Prepare the battle dictionary
#             battle_dict = {
#                 'topic': topic,
#                 'total_rounds': total_rounds,
#                 'pro-prompt': {
#                     'prompt': top_prompts_pro[i],
#                     'initial_elo': initial_elo_rating_1,
#                     'final_elo': final_elo_rating1
#                 },
#                 'con-prompt': {
#                     'prompt': top_prompts_con[j],
#                     'initial_elo': initial_elo_rating_2,
#                     'final_elo': final_elo_rating2
#                 },
#                 'pro_arguments': result[4] if result else [],
#                 'con_arguments': result[5] if result else [],
#                 'winner_role': winner,
#                 'winner_prompt': top_prompts_pro[i] if winner == 'pro' else 'draw' if winner == 'draw' else top_prompts_con[j]
#             }

#             league_dictionary[league_number][battle_number] = battle_dict

#         end_time = time.time()
#         print(f"Time taken to complete the final league {z}: {end_time - start_time} seconds")
...
# def update_elo(dict, pro_prompt, con_prompt, winner):
...
#     if winner == "draw":
#         # Find the ELO scores of the winner and loser
#         winner_rating = dict[pro_prompt]
#         loser_rating = dict[con_prompt]
#         # Calculate the expected score of the winner and loser
#         expected_winner = 1 / (1 + 10 ** ((loser_rating - winner_rating) / 400))
#         expected_loser = 1 / (1 + 10 ** ((winner_rating - loser_rating) / 400))
#         # Update the ELO ratings of the winner and loser
#         winner_rating = winner_rating + 40 * (0.5 - expected_winner)
#         loser_rating = loser_rating + 40 * (0.5 - expected_loser)
#         # Update the ELO ratings in the dictionary
#         dict[pro_prompt] = winner_rating
#         dict[con_prompt] = loser_rating
...
#         expected_loser = 1 / (1 + 10 ** ((winner_rating - loser_rating) / 400))
#         # Update the ELO ratings of the winner and loser
#         winner_rating = winner_rating + 40 * (1 - expected_winner)
#         loser_rating = loser_rating + 40 * (0 - expected_loser)
#         # Update the ELO ratings in the dictionary
#         dict[pro_prompt] = winner_rating
#         dict[con_prompt] = loser_rating
...
#     elif winner == "con":
#         # Find the ELO scores of the winner and loser
#         winner_rating = dict[con_prompt]
#         loser_rating = dict[pro_prompt]
#         # Calculate the expected score of the winner and loser
#         expected_winner = 1 / (1 + 10 ** ((loser_rating - winner_rating) / 400))
#         expected_loser = 1 / (1 + 10 ** ((winner_rating - loser_rating) / 400))
#         # Update the ELO ratings of the winner and loser
...
#         loser_rating = loser_rating + 40 * (0 - expected_loser)
#         # Update the ELO ratings in the dictionary
#         dict[con_prompt] = winner_rating
#         dict[pro_prompt] = loser_rating
...
def main():
    ...
    for no_of_prompts_start in range (15,20,2):
        for no_of_prompts_between in range (15,22,2):
            for total_rounds in range (3,4,1):
                for k in range (6,math.floor(no_of_prompts_between/2),1):
                    for temperature in temp:
                        for max_token in max_tokens:
                                ...
                                folder_name = "no_of_prompts_start_"+str(no_of_prompts_start)+"_no_of_prompts_between_"+str(no_of_prompts_between)+"_total_rounds_"+str(total_rounds)+"_k_"+str(k)+"_temperature_"+str(temperature)+"_max_token_"+str(max_token)+"elo_rating_weight_40"
                                ...
                                tournament(no_of_topics, no_of_prompts_start, no_of_prompts_between, total_rounds, k, 10, temperature, max_token, current_directory)
                                # Dictionary to store the ELO ratings of the prompts
                                elo_ratings_dict = {}
                                # Dictionary to store the history of the games in the leagues
                                league_dict = {}
